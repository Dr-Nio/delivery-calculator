 ...
 */
function getWeightRate(weightKg) {
    if (weightKg <= 5)
        return 350;
    if (weightKg <= 15)
        return 250;
 ...
 */
function getDistanceRate(distanceKm) {
    if (distanceKm <= 10)
        return 1000;
    if (distanceKm <= 30)
        return 1500;
 ...
 * - capped max fee
 */
export function calculateShipping({ weightKg, origin, destination, maxCap = 10000, // Maximum allowed delivery fee
 }) {
